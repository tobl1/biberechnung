...
export function defaultState(): GameState {
  ...
  return { players, scores: Object.fromEntries(players.map((p) => [p.id, 0])), theme: DEFAULT_THEME, game: DEFAULT_GAME, font: DEFAULT_FONT }
}
...
export function loadState(): GameState {
  try {
    ...
    return { players: parsed.players, scores: parsed.scores ?? {}, theme: parsed.theme ?? DEFAULT_THEME, game: parsed.game ?? DEFAULT_GAME, font: parsed.font ?? DEFAULT_FONT }
  } catch {
    return defaultState()
  }
}
